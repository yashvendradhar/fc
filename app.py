...
st.set_page_config(
    page_title="FC Derby Arena • Matchmaker",
    page_icon="⚽",
    layout="wide",
...
# Match Stats Expander
try:
    with open("match_details.json", "r", encoding="utf-8") as f:
        history = json.load(f)
        total_games = history[-1]["gameCount"] if history else 0
        last_played = history[-1]["timeStamp"] if history else "N/A"

    st.write("")
    st.markdown("---")
    stat_col1, stat_col2, stat_col3 = st.columns(3)
    with stat_col1:
        st.metric("🔥 Total Fixtures", total_games)
    with stat_col2:
        st.metric("🕒 Last Kickoff", last_played)
    with stat_col3:
        st.metric("⚡ Prime Derby Mode", "Active")
except Exception:
    pass